(() => {
  ...
  const API_BASE = "https://mental-health-predictor-bxma.onrender.com";
  ...
  function renderResult(score) {
    ...
    if (interpretationEl) {
      ...
      // Highlight appropriate interpretation
      const items = interpretationEl.querySelectorAll(".interpretation-item");
      items.forEach(item => item.classList.remove("highlighted"));
      
      let highlightClass;
      if (clamped >= 7) highlightClass = "excellent";
      else if (clamped >= 5) highlightClass = "good";
      else if (clamped >= 3) highlightClass = "fair";
      else highlightClass = "concerning";
      
      const highlightEl = interpretationEl.querySelector(`.${highlightClass}`);
      if (highlightEl) highlightEl.classList.add("highlighted");
    }
